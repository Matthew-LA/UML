...
//Preconditions: None
//Postconditions: Allocate space for a string that represents the empty
...
//Precondition: phMY_string holds the aaddress of a valid handle to MY_STRING
// object.
//Postcondition: The memory used for the MY_STRING object has to be reclaimed by
// the system and the handle referred to by pointer phMY_string has been set to NULL.
...
// As an example, the string "the" would be set capacity at 4 instead of 3. A
...
//Postcondition: Returns a copy of the ineger value of the object's capacity.
int my_string_get_capacity(MY_STRING hMy_string);

//Precondition: hMy_string is the handle of a valid My-string object.
//Postcondition: Returns a copy of the integer value of the objects size.
int my_string_get_size(MY_STRING hMy_string);

//Precondition: hLeft_string and hRight_string are valid My_string onjects.
//Postcondition: returns an integer less than zero if the string represented
// b hLeft_string is lexicographically smaller than hRight_string. If one
// string is a prefix of the other string then the shorter string is considered
// to be the smalller one. (app less than apple). Returns 0 if the strings are
// the same and returns a number greater than zero if the string is represented
// by the hLeft_string is bigger than the right.
int my_string_compare(MY_STRING hLeft_string, MY_STRING hRight_string);

//Preconditions: hMy_string is the handle to a valid My_string object.
//Postconditons: hMy_string will be the handle of a string object that contains
// the next string from the file stream fp according to the following rules.
// 1)Leading white space will be ignored.
// 2)All characters (after the first non-whitespace character is obtained
//   and included) will be added to the string will continue to grow as needed
//   until all characters are stored.
// 3)A stopping condition is met if we read a whitespace character after we have
//   read at least one non-whitespace character or if we reach the end of file.
// Function will return SUCCESS if a non-empty string is read successfully.
// and a failure otherwise. Remeber that the incoming string may already contain
// some data and this function should replace the data but not necessarily
// resize the array unless needed.
...
-UU-:----F1  my_string.h    Bot L101   (C/l Abbrev) -----------------------------------------------------------------------
