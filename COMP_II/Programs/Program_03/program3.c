...
//  COMPII
//
//  Created by Matthew Lorette Anaya on 4/24/20.
...
Program: 3
 ...
 Date: 4/24/2020
Time spent: 16 hours
 Purpose: Sort a large set of numbers from an input to have
        no duplicates.
***********************************************************************/
#include <stdio.h>
#include <stdlib.h>
...
void bubble_sort(ARRAY hTemp_array, int size);
...
int main()
{
    ...
    bubble_sort(hmy_array, size_temp);
...
void bubble_sort(ARRAY hTemp_array, int size)
{
    Array* pTemp_array = (Array*)hTemp_array;
    int swapped = 1, i = 1;
    for (int n = 0; n < size - 1 && swapped; i++) {
        swapped = 0;
        for (i = 0; i < size - 1; i++) {
            if (pTemp_array->data[i] > pTemp_array->data[i + 1]) {
                swap(&pTemp_array->data[i], &pTemp_array->data[i + 1]);
                swapped = 1;
            }
        }
    }
